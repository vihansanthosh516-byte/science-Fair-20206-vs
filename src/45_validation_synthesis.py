...
# Phase 2b / Phase 3 artifacts (generated by src/46 and src/47)
...
def build_master_summary(loaded: Dict[str, Any],
    ...
    # Phase 2b (Sobol) + Phase 3 (dual-drug / 3-arm) enrichment
    sobol_block, dual_block = _load_phase23_artifacts()
    if sobol_block is not None:
        master["sobol_sensitivity"] = sobol_block
    if dual_block is not None:
        master["three_arm_comparison"] = dual_block
    return master


def _load_phase23_artifacts():
    """Load Sobol results (src/46) and 3-arm comparison (src/47) if present."""
    sobol_block = None
    dual_block = None
    if SOBOL_RESULTS_JSON.exists():
        with open(SOBOL_RESULTS_JSON) as f:
            sobol_block = json.load(f)
        print(f"[Phase2b] Loaded Sobol results -> {SOBOL_RESULTS_JSON}")
    if DUAL_DRUG_JSON.exists():
        with open(DUAL_DRUG_JSON) as f:
            raw = json.load(f)
        pts = raw.get("patients", [])
        # Summary statistics across 8 patients for each arm
        def _arm_summary(patients, arm):
            arr = lambda k: np.array([p[arm][k] for p in patients], dtype=float)
            ttp, auc, rf = arr("ttp_days"), arr("auc"), arr("resistant_fraction")
            return {
                "ttp_days_mean": float(ttp.mean()),
                "ttp_days_std": float(ttp.std(ddof=1)),
                "auc_mean": float(auc.mean()),
                "auc_std": float(auc.std(ddof=1)),
                "resistant_fraction_mean": float(rf.mean()),
                "resistant_fraction_std": float(rf.std(ddof=1)),
            }
        dual_block = {
            "parameters": raw.get("parameters", {}),
            "arms": {
                "mtd": _arm_summary(pts, "mtd"),
                "single_adaptive": _arm_summary(pts, "single_adaptive"),
                "dual_adaptive": _arm_summary(pts, "dual_adaptive"),
            },
            "patients": pts,
        }
        # Drug exposure reduction vs MTD
        mtd_auc = np.array([p["mtd"]["auc"] for p in pts])
        sa_auc = np.array([p["single_adaptive"]["auc"] for p in pts])
        da_auc = np.array([p["dual_adaptive"]["auc"] for p in pts])
        dual_block["drug_reduction"] = {
            "single_vs_mtd_mean": float(np.mean(1.0 - sa_auc / mtd_auc)),
            "dual_vs_mtd_mean": float(np.mean(1.0 - da_auc / mtd_auc)),
        }
        print(f"[Phase3] Loaded 3-arm comparison -> {DUAL_DRUG_JSON}")
    return sobol_block, dual_block
...
def render_master_canvas(master: Dict[str, Any],
    ...
    # ----------------------------------------------------------------- #
    # Panel D: Drug reduction (left) + Sobol tornado / 3-arm (right)
    # ----------------------------------------------------------------- #
    ss_d = gs[1, 1]
    sub_d = ss_d.subgridspec(1, 2, wspace=0.30)

    # D-left: Cohort drug reduction by inflammation tier (existing chart)
    ax_d = fig.add_subplot(sub_d[0, 0])
    ...
    ax_d.set_title(f"D1. Drug-Toxicity Reduction: "
    ...
    ax_d.legend(handles + [plt.Line2D([0], [0], color="navy", ls="--",
                                          lw=1.2)],
                  ["Low", "Mid", "High", f"Mean = {mean_pct:.1f}%"],
                  fontsize=7, loc="lower right")
    ax_d.grid(alpha=0.3, axis="y")

    # D-right: Sobol tornado plot (Phase 2b) embedded from PNG
    ax_d2 = fig.add_subplot(sub_d[0, 1])
    if SOBOL_TORNADO_PNG.exists():
        import matplotlib.image as mpimg
        img = mpimg.imread(str(SOBOL_TORNADO_PNG))
        ax_d2.imshow(img)
        ax_d2.set_title("D2. Sobol Sensitivity (TTP variance decomposition)",
                          fontsize=9, fontweight="bold", loc="left")
        ax_d2.axis("off")
    else:
        ax_d2.text(0.5, 0.5, "Sobol tornado plot unavailable\n"
                     "run src/46_sensitivity_analysis.py",
                     transform=ax_d2.transAxes, ha="center", va="center",
                     fontsize=9)
        ax_d2.set_title("D2. Sobol Sensitivity (unavailable)",
                          fontsize=9, fontweight="bold", loc="left")
        ax_d2.axis("off")
